...
<template>
    <div class="stat-grid">
        <div class="card">🍌 Net Carbs <span class="value">{{ totals.netCarbs.toFixed(1) }}g</span></div>
        <div class="card">🍌 Total Carbs <span class="value">{{ totals.totalCarbs.toFixed(1) }}g</span></div>
        <div class="card"><span>🥑 Fat</span> <span class="value">{{ totals.fat.toFixed(1) }}g</span></div>

        <div class="card fat-pct" :data-status="fatStatus">
            <span class="label">🥑 % Calories from Fat</span>
            <span class="value">{{ percentageOfCaloriesFromFat.toFixed(1) }}%</span>
        ...
        <div class="card">🫘 Protein <span class="value">Not tracked yet...</span></div>
        <!--<div class="card">🫘 Protein <strong>{{ totals.protein.toFixed(1) }}g</strong></div>-->
        <div class="card">🥦 Fibre <span class="value">{{ totals.fibre.toFixed(1) }}g</span></div>
        <div class="card">🔥 Calories <span class="value">{{ totals.calories.toFixed(0) }}</span></div>
    </div>
</template>
...
<style scoped lang="scss">
    .stat-grid {
        ...
        /*.card {*/
            ...
            /*}*/
            /*.value {*/
                /*font-variant-numeric: tabular-nums;  !* ✅ aligns digits/decimals *!*/
                /*font-weight: 600;*/
            /*}*/
        /*}*/
        ...
        .fat-pct {
            display: grid;
            grid-template-columns: 1fr auto; /* label | value (right-aligned by .value rule) */
            row-gap: .35rem;
            /* keep the percentage value aligned to the right on the first line */
            > .value { align-self: flex-end; }

            .target-bar,
            .ticks-row { grid-column: 1 / -1; } /* bar + labels span both columns */
